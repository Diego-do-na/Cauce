...
# REPO_ROOT is the git repository root (the whole product repo Cauce was
# dropped into — src/, docs/, whatever else the project has) — git plumbing
# (pull/push/commit) always operates there. Everything that is purely
# Cauce's own state lives under ORCHESTRATION_ROOT instead, so the task
# board, scripts, dashboards and this tool's own secrets stay out of the
# product-code tree the agents work in.
...
def task_worktree_path(task_id: str) -> Path:
    """The one place that spells out claim_task.py's own worktree naming
    convention (`../task-<id>`, a sibling of REPO_ROOT) -- so
    cleanup_task_worktree, task_log.py, and finish_task.py's test gate all
    agree on it instead of each re-deriving the same path independently."""
    return REPO_ROOT.parent / f"task-{task_id}"
...
def run_declared_tests(task: dict[str, Any], cwd: Path) -> subprocess.CompletedProcess | None:
    """Runs whatever tests a task declared for itself in its own
    `test_paths`. Returns None (nothing to run, nothing to block on) if
    the task declared none.

    test_paths is task-declared, not auto-detected: whoever writes the
    task (usually the model generating the initial board from the
    project's spec) decides, task by task, which units of work are worth
    a real test run -- e.g. "measure this endpoint's latency" gets one,
    "scaffold a DB migration" doesn't need one. Cauce itself never guesses
    at this from a repo layout.

    The runner is `CAUCE_TEST_CMD` (default "pytest") -- swappable per
    project via one env var so a non-Python stack (`cargo test`,
    `npm test --`) never requires touching Cauce's own code, only that one
    setting in orchestration/.env.

    Runs in `cwd`, which finish_task.py passes as the task's own worktree
    -- it already has whatever the agent installed there, so this is
    faster and more representative than spinning up a fresh throwaway
    clone that would need everything reinstalled first.

    A hard timeout (CAUCE_TEST_TIMEOUT_S, default 300s) guards against a
    hung test (a server that never comes up, an infinite loop) blocking
    finish_task.py forever -- surfaced as a CauceError, same as any other
    fatal problem finish_task.py can hit.
    """
    test_paths = task.get("test_paths") or []
    if not test_paths:
        return None
    test_cmd = os.environ.get("CAUCE_TEST_CMD", "pytest")
    timeout = float(os.environ.get("CAUCE_TEST_TIMEOUT_S", DEFAULT_TEST_TIMEOUT_S))
    command = shlex.split(test_cmd) + list(test_paths)
    try:
        ...
        raise CauceError(
            f"task tests timed out after {timeout:.0f}s ({' '.join(command)}) in {cwd} "
            f"-- a hung test/server blocks finish just like a real failure would; "
            f"fix it or raise CAUCE_TEST_TIMEOUT_S"
        ) from exc
    except OSError as exc:
        raise CauceError(f"could not run task tests ({' '.join(command)}) in {cwd}: {exc}") from exc
