...
# test-paths reminder, if the task declared any) for the given task,
# straight from tasks.yaml. Empty output (no error) if anything goes
...
if task:
    ...
    if test_paths:
        ...
        prompt += (
            "\n\nBefore finishing, run and satisfy these declared tests "
            f"(finish_task.py will re-run them and refuse to merge if they fail):\n{listed}"
        )
    print(prompt)
PY
}
