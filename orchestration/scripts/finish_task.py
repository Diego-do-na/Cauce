...
Flow:
    ...
    3. If the task declared its own `test_paths` (not every task needs
       one — a DB-scaffolding task doesn't, a "measure this endpoint's
       latency" task should): run them (CAUCE_TEST_CMD, default "pytest")
       in the task's own worktree. A failure blocks exactly like a merge
       conflict below — nothing pushed, task stays claimed, fix and re-run.
    4. check_merge.py: would `branch` merge cleanly into main?
...
def main() -> None:
    ...
    try:
        ...
        test_result = None
        if task.get("test_paths"):
            worktree = task_worktree_path(task_id)
            if not worktree.exists():
                fail(
                    f"task {task_id} declared test_paths {task['test_paths']!r} but its "
                    f"worktree ({worktree}) doesn't exist — can't run them. If it was "
                    f"already cleaned up, something is inconsistent; investigate by hand."
                )
                return
            print(f"Running {task_id}'s declared tests in {worktree}...")
            test_result = run_declared_tests(task, worktree)
